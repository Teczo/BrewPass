...
describe("countLeadingNoVendorFailures (Phase O.2 §2.2)", () => {
  const noVendor = { status: "failed" as const, failureReason: "no_vendor_available" };
  const delivered = { status: "delivered" as const };
  const otherFail = { status: "failed" as const, failureReason: "charge_failed" };

  it("counts the leading run and stops at the first non-matching order", () => {
    expect(countLeadingNoVendorFailures([noVendor, noVendor, delivered, noVendor])).toBe(2);
    expect(countLeadingNoVendorFailures([])).toBe(0);
    expect(countLeadingNoVendorFailures([delivered, noVendor])).toBe(0);
    // A different failure reason breaks the run (only coverage gaps count).
    expect(countLeadingNoVendorFailures([noVendor, otherFail, noVendor])).toBe(1);
  });

  it("escalates when the current failure plus the prior run reaches the threshold", () => {
    // Current failure (+1) plus 2 prior no-vendor days = 3 → escalate.
    expect(1 + countLeadingNoVendorFailures([noVendor, noVendor])).toBeGreaterThanOrEqual(
      COVERAGE_GAP_ESCALATION_THRESHOLD,
    );
    // Current failure plus only 1 prior = 2 → no escalation yet.
    expect(1 + countLeadingNoVendorFailures([noVendor, delivered])).toBeLessThan(
      COVERAGE_GAP_ESCALATION_THRESHOLD,
    );
  });
});

describe("buildUnroutedOrder (Phase O.2 §2.2)", () => {
  it("snapshots drink + location but carries no vendor, price, or assignment", () => {
    const sub = makeSubscription();
    const pref = makePreference(sub.userId);
    const location = makeLocation(sub.userId);

    const order = buildUnroutedOrder({
      subscription: sub,
      preference: pref,
      location,
      localDate: WEDNESDAY,
    ...
    expect(order.priceSen).toBeUndefined();
    expect(order.assignmentMethod).toBeUndefined();
    expect(order.autoGenerated).toBe(true);
    // Drink + location are still snapshotted (rule #6), by copy.
    expect(order.drink).toEqual(pref.defaultDrink);
    expect(order.drink).not.toBe(pref.defaultDrink);
    expect(order.location.label).toBe("Office");
    // Same cutoff/delivery instants as a routed order on this day.
    expect(order.cutoffAt.toISOString()).toBe("2026-06-09T22:00:00.000Z");
    expect(order.deliverAt.toISOString()).toBe("2026-06-10T00:00:00.000Z");
  });
});
