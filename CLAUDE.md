...
> **Current direction (as of 2026-06):** J and K are complete. **L is parked** — it is gated on multi-stop courier capability that has not been validated, and is not a current priority. Build order is **finish remaining J → K → M, with L deferred until courier multi-stop is confirmed.** The J.6 → L forward-reference (member tracking must not assume one delivery = one order) stays in the spec so L stays cheap to pick up later, but nothing else L-related is built. **M is the active phase** and does not depend on L.
