...
- **O.2 — Routing no-vendor lifecycle (`fallbacks.md` §2.2, §2.5). ✅ DONE.** §2.2 (behavior change confirmed under rule #11) and §2.5 both built.
  - **§2.2 (built):** `Order.vendorId` is now **optional** (the schema ripple was guarded across routing-capacity aggregation, admin, v1 order routes, payout, delivery runs, webhook emit — every site that reads a vendor operates on orders that always have one by that lifecycle point). On a **transient** no-vendor day (coverage exists but every vendor is busy/closed/full), generation persists a `scheduled` order with **no vendor** (`buildUnroutedOrder`) instead of skipping; a true **coverage gap** still waitlists (O.1) and never makes a vendorless order. `reattemptUnroutedOrders` (wired into the nightly cron before notification) re-routes still-unrouted orders each pass, snapshotting vendor+price+capacity idempotently. The night-before reminder skips vendorless orders. At cutoff, a still-unrouted order is **failed `no_vendor_available`, never charged**, and the user is notified (plan stays active). A run of **`COVERAGE_GAP_ESCALATION_THRESHOLD` (3)** consecutive `no_vendor_available` failures sets `Order.coverageGapEscalated`, surfaced in the admin failures panel. Pure helpers (`buildUnroutedOrder`, `countLeadingNoVendorFailures`, `hasAreaCoverage`) are unit-tested. Scope note: applied to **personal** orders only; office/corporate keeps its current skip (avoids touching the company-card path).
  - **§2.5 (built):** a vendor going offline **after charge, before delivery** is handled **event-driven** (no new cron) — the vendor-profile and admin vendor-status endpoints call `reassignChargedOrdersForOfflineVendor` on the `active → paused/offline/suspended` edge (`vendorWentOffline`, unit-tested). It rescues each `confirmed`/`preparing` charged order (excluding Pack-pinned, rule #20) by reassigning to the best eligible vendor at the **snapshotted `priceSen`** — the customer's charge is unchanged and the commission split is recomputed at the rescue vendor's rate, so the **platform absorbs the rate delta** (rule #6). The rescue **bypasses the per-vendor accept-cutoff** (new `RoutingRequest.bypassAcceptCutoff`, unit-tested) since it's intentionally a same-day post-cutoff reroute; all other eligibility still applies. If no vendor can be found, the order is marked `failed`/`vendor_offline_no_reassignment` and **auto-refunded in full** (idempotent `refundFailedDelivery`; the offline vendor was never paid — delivery-gated, rule #4), with an apology to the customer (and the team owner for office coffee). Every mutation is claimed on `(vendorId, status)` so a re-fire or concurrent call can't double-reassign or double-refund.
- **O.3 — Charge-retry cadence (`fallbacks.md` §4.1, §4.2). ✅ DECIDED — interim retry-once on Hobby; retry-cron deferred to Pro.** `fallbacks.md` and CLAUDE rule #3/#11 specify **"3× over ~10 min (≈ 0, 3, 10 min)"**; the current code (`charge.ts`) is **retry-*once* in-process, then skip**. A literal "0/3/10 min" schedule cannot run inside one cutoff invocation (`maxDuration` is 300 s = 5 min) and the Hobby plan allows only daily cron, so the true cadence needs a sub-daily **retry cron** (a `charge_retry` sweep over `chargeStatus:"retrying"` orders with `nextRetryAt`/`chargeAttempts`), which adds a cron (rule #12) and only runs on Pro. **Decision (2026-06): stay on Hobby until there are real customers, so keep retry-once as the documented interim** — do **not** claim 3× anywhere. The retry cron is the planned Pro-upgrade path and is recorded in the Go-Live TODO. Everything else in §4 (idempotent keys, subscription-stays-active, company-card multi-person blast-radius notify to owner) is already in place — keep it.
- **O.4 — Stuck-delivery sweeper & live-tracker degradation (`fallbacks.md` §5.5, §5.6). ✅ DONE.** Net-new. Built: pure `isDeliveryStuck` + `findStuckDeliveries` in `src/lib/delivery/stuck.ts` (in-flight `assigned`/`picked_up` past **`STUCK_DELIVERY_THRESHOLD_MS` = 2× `STUCK_DELIVERY_EXPECTED_MS` (45 min)** = 90 min, anchored on the most specific in-flight timestamp; unit-tested). Detection is **read-time on the admin dashboard render** (no new cron — rule #12). A **Stuck deliveries** admin panel surfaces each with a **Resolve** action: re-dispatch (`courier_redispatch`), mark delivered (`courier_mark_delivered`), or mark failed (new `adminMarkFailed` → `courier_mark_failed`, which runs the same idempotent failed-transition + auto-refund as a courier-failure webhook). Tracker (§5.5/§12.4): driver location **>60 s stale** (computed at poll time, not render) surfaces a prominent "Live location is delayed — open in {provider} tracker" link while polling continues; the static-map `<img>` gets an `onError` non-map fallback for when Maps is down/blocked. The expected-duration threshold is a tunable detection constant (never touches money/state machine), so it's not a rule #11 business rule.
- **O.5 — Per-panel render resilience (`fallbacks.md` §13). ✅ DONE.** Net-new (only a single `global-error.tsx` existed before). Built: a reusable client **`ErrorBoundary` + `PanelError`** primitive (`src/components/ui/error-boundary.tsx`, Sentry-reporting, retry-by-re-render) wrapping the interactive **client** panels on the dashboard (`UpcomingOrder`, `DeliveryTracker`, `OverlapNotice`) and admin (`StuckDeliveries`, orders/vendors/users tables, commission panel) so one failed panel shows a compact "couldn't load — refresh" tile while the rest render (§13.1/§13.2). Server-rendered panel errors are caught by new **route-level `error.tsx`** for `dashboard`/`admin`/`vendor`/`onboarding`, with **`loading.tsx`** skeletons for `dashboard`/`admin` (§13.2) — the two layers together mean no failure ever blanks the app. **§13.3 image fallback:** the only `<img>` rendered anywhere is the delivery-tracker static map, which already has an `onError` non-map fallback (O.4) — no vendor-logo/menu/driver images are rendered, so a placeholder component would be unused (rule #15, not built); noted here so a future image render adds the fallback at that site.
- **O.6 — Corporate, auth, geo & webhook tail (`fallbacks.md` §8.4, §8.8, §11.1/§11.3, §12, §9). ✅ DONE.** Built/verified:
  - **§8.4/§8.8 (built):** owner **setup checklist** on `corporate-owner-dashboard` (company card / office defaults / [bundle coffee in bundle mode] / join code, with ✓/○ state) shown until setup is complete, plus the generation-blocker copy ("Office coffee starts once you set …" + the bundle-mode-no-drink prompt). Pure UI surfacing of the same skip reasons the 8 PM cron already enforces server-side — no schema/behavior change.
  - **§11.3 (built):** the landing page wraps `getSession()` so an Auth0/session hiccup can never break it — it falls back to the signed-out view with a "sign-in is having a moment, try again shortly" note; already-authenticated users keep working from the session cookie.
  - **§12.1/§12.2 (built — friendly copy):** the vendor-apply and subscriber-location geocode-miss responses now return clear, recoverable guidance (check the address / use GPS "Use my current location"). The current behaviour is non-breaking by design (a clear actionable 422, not a crash).
  - **§11.1, §9.1, §12.3 (verified — already built):** new users default to `role: "individual"` and are sent to onboarding (`users.ts`); the outbound-webhook dispatcher already retries with exponential backoff `[1, 5, 15, 60, 180, 360]` min over `WEBHOOK_MAX_ATTEMPTS = 6` to a terminal `failed` state (`dispatch.ts` / `signing.ts`); and "Use my current location" failure already falls through to manual address entry (`locations-manager.tsx`, never blocks).
  - **Deferred (documented, not regressions):** the §12.1/§12.2 *softer* "save the address `unverified` with `lat/lng = null` and let admin/manual-pin sort it out" path needs routing to tolerate a null `geo` (the same geo-optional ripple as the O.2 `vendorId` work) — not worth that risk for a rare geocode miss the app already degrades on gracefully. §9.2 (the `*/5` webhook sweep cron) stays a Go-Live TODO (Hobby).
...
- [ ] **Add the charge-retry cron (Phase O.3 §4.1/§4.2).** Today a failed
  cutoff charge is **retried once in-process, then skipped** (`charge.ts`).
  The spec's "3× over ~10 min (≈ 0, 3, 10 min)" needs a sub-daily sweep,
  which Hobby can't schedule. When upgrading to Pro: add a `charge_retry`
  cron (every ~3–5 min) that re-attempts orders left in a `retrying` charge
  state, with `chargeAttempts`/`nextRetryAt` order fields and the same Stripe
  idempotency key (`coffee_<orderId>`) so retries never double-charge. Applies
  identically to personal and company-card charges. Until then, retry-once is
  the documented interim — do not claim 3×.
